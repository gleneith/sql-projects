...
-- adding a foreign key to the shop table 
alter table shops
...
rename column coffe_type to coffee_type;

-- inserting first two rows into the employee table
...
-- inserting 'common grounds' into shop table
...
-- inserting 'los angeles' into the locations table 
insert into locations values (1, 'Los Angeles', 'United States');

-- updating the city_id in the shops table for coffeeshop_id = 1 to 1
update shops 
set city_id = 1
where coffeeshop_id = 1;

-- inserting the first two values where coffeshop_id = 1 into the suppliers tables 
...
-- inserting into the shops table
...
-- inserting into into the suppliers table
...
-- the employees who make more than 50k 
select * 
from employees
where salary > 50000

--  the employees who work in 'common grounds' coffeeshop
select * 
from employees
where coffeeshop_id = 1;

-- the employees who work in 'common grounds' and make more than 50k
select * 
from employees
where coffeeshop_id = 1 and salary >50000;

-- the employees who work in 'common grounds' or more than 50k
select * 
from employees
where coffeeshop_id = 1 or salary >50000;

-- the employees who work in 'common grounds, make more than 50k and are males 
select * 
from employees
where coffeeshop_id = 1 and salary >50000 and gender = 'M';

-- in | not in | is null | between queries 
...
-- all 'robusta' and arabica' coffee type
select * 
from suppliers
...
-- employees whose email addresses aren't missing 
select * 
from employees
where email is not null; 

-- employees who make between 35k and 50k
select * 
from employees
...
-- retun all unique coffeshop ids
...
-- renaming columns
select email, email as email_address,
...
-- returning all emails and the length of them
...
-- concatenating the first and last names to create a full name 
select first_name, last_name,
concat(first_name,' ' , last_name, ' ') as full_name
from employees;

-- concatenating columns to create a sentence
...
-- if the email has '.com', return true, otherwise false
...
-- position 
-- finding the position of '@' in the email
...
-- the difference between maximum and minimum salary 
select max(salary) - min(salary)
from employees;

-- selecting the average salary 
select avg(salary)
from employees;

-- round average salary to the nearest integer
...
-- returning the number of employees, the avg & min & max total salaries for each coffeeshop 
select coffeeshop_id, count(*) as numb_of_emp, round(avg(salary),0) as avg_salary,
			 min(salary) as min_sal, max(salary) as max_sal, sum(salary) as total_sal
 from employees
 group by coffeeshop_id
 order by numb_of_emp desc;
 
-- having 
-- after group by, returning only the coffeeshop with more than 200 employees
select coffeeshop_id, count(*) as numb_of_emp, round(avg(salary),0) as avg_salary,
			 min(salary) as min_sal, max(salary) as max_sal, sum(salary) as total_sal
 from employees
 group by coffeeshop_id
 ...
 -- after group by, returning only the coffeeshop with a minimum salary of less than 10k
 select coffeeshop_id, count(*) as numb_of_emp, round(avg(salary),0) as avg_salary,
			 min(salary) as min_sal, max(salary) as max_sal, sum(salary) as total_sal
 from employees
 group by coffeeshop_id
...
-- case with group by & transporting data

-- case 
-- if pay is less than 50k, the low pay, otherwise high pay
select employee_id, first_name, last_name, salary,
case 
...
-- if the pay is less than 20k, then low pay, if between 20k - 50k inclusive, the medium pay 
-- if over 50k, the high pay
select employee_id, first_name, last_name, salary,
case 
...
-- case & group by 
-- returning the count of employees in each pay category 
...
select 
			...
			sum(case when salary between 20000 and 500000 then 1 else 0 end) as medium_pay,
...
-- deleting the values that were just created when using join 
delete from locations where city_id = 4;
delete from shops where coffeeshop_id = 6;

-- union

-- returning all the cities and countries 
...
-- using union all to keep all duplicates 
select country from locations
union all
select country from locations; 

-- returning all coffeeshop names, cities and countries 
...
select first_name, last_name, salary,
				(select round(avg(salary),0) from employees limit 1) as avg_salary 
from employees;
